...
import { WorkItem, WorkItemType, Priority, Status, IssueType, IssueTypeLabels, useGetWorkItemsByUserQuery, useGetTeamsQuery, useGetUsersQuery, useGetProgramsQuery } from '@/state/api';
...
const IssuesPage = () => {
    ...
    // First filter by work item type, then apply issue type filter, then program filter, then team filter, then status filter
    const typeFilteredWorkItems = workItems?.filter((workItem: WorkItem) => workItem.workItemType === WorkItemType.Issue);
    ...
    const programFilteredWorkItems = selectedProgramId === "all"
        ? issueTypeFilteredWorkItems
        : issueTypeFilteredWorkItems?.filter((item) => item.programId === selectedProgramId);
    ...
    return (
        <div className="m-5 p-4">
            ...
            <div className="mb-6 rounded-lg bg-white p-4 shadow dark:bg-dark-secondary">
                <div className="mb-4 flex items-center justify-between">
                    <h3 className="text-lg font-semibold dark:text-white">
                        {selectedPriority === "all"
                            ? "All Issues"
                            : `${selectedPriority} Priority Issues`}
                    </h3>
                    <select
            ...
            <div className="mb-6 rounded-lg bg-white p-4 shadow dark:bg-dark-secondary">
                ...
                <BurndownChart workItems={issueTypeFilteredWorkItems || []} isDarkMode={isDarkMode} />
            </div>
